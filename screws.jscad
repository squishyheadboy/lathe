function main(){
	var things = Array();
	things.push(screw_MX(3,10).translate([-10,0,0]));
	things.push(screw_MX(4,20));
	things.push(screw_MX(5,20).translate([10,0,0]));
	things.push(screw_MX(6,20).translate([20,0,0]));
	return things;
}

function screw_MX(diam,length)
{
	var head_chamfer=0.5;
	var screw_size = diam;
	var head_radius = 0.75*screw_size;
	var screw = CSG.cylinder({start:[0,0,-length],end:[0,0,0],radius:screw_size/2})
				.union(CSG.cylinder({start:[0,0,0],end:[0,0,screw_size-head_chamfer],radius:head_radius}))
				.union(CSG.cylinder({start:[0,0,0],end:[0,0,head_chamfer],radiusStart:head_radius,radiusEnd:head_radius-head_chamfer/2}).translate([0,0,screw_size-head_chamfer]))
				.subtract(CSG.cylinder({start:[0,0,1],end:[0,0,screw_size+1],radius:screw_size/2,resolution:6}))
